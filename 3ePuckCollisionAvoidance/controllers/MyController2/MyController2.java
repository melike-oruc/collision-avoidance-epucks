...
public class MyController2 {
  ...
  static GPS  gps;
  static Node puckone;
  static Node puckthree;
  static Node midTarget1;
  static Node midTarget2;
  ...
  public static void main(String[] args) {
    ...
    int timeStep = (int) Math.round(robot.getBasicTimeStep());
     ...
     puckthree = robot.getFromDef("puckthree");
     puckone = robot.getFromDef("puckone");//getting node of puckone
     midTarget1 = robot.getFromDef("midTarget1");
     midTarget2 = robot.getFromDef("midTarget2");
     ...
     if (robot_name.equals("pucktwo"))
     {
       //target[0] = -0.4;
       //target[1] = -0.5;
       target[0] =  midTarget1.getValues()[0];
       target[1] =  midTarget1.getValues()[1];
     }
    ...
    while (robot.step(timeStep) != -1) {
      if(check_other_robot() || check_puckthree_robot())//checking if there is another robot that will collide 
      {
        move_robot(0,0);
      }
      else
      {
        switch(state) 
        {
          case "forward":
            if (at_target())
            {
              if(target[0] == -0.4 && target[1] == -0.5)
              {
              target[0] = 0.1;
              target[1] = 0.2;
              }
              else if(target[0] == 0.1 && target[1] == 0.2)
              {
               target[0] = -1.3;
               target[1] = 1.3;
              }
              else if(target[0] == -1.3 && target[1] == 1.3)
              {
                move_robot(0,0);
               state = "stop";
              }
  ...
  static void move_robot(double ls, double rs)
   ...
   static boolean obstacal_found()
    ...
    static boolean side_obstacal_found()
    {
      for(int i=0; i<2;i++)
       {
   ...
   static boolean check_puckthree_robot()//avoiding pucktwo
   {
     double[] values = puckthree.getPosition();
     double x = gps.getValues()[0];
     double y = gps.getValues()[1];
     /*
     if (values[0]>0 && values[1]>0)
